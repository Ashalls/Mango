...
/**
 * Coerce 24-char hex strings to ObjectId — but ONLY under an `_id` path, so a
 * field that legitimately stores a hex STRING isn't turned into an ObjectId
 * (which would then never match). Explicit ObjectId filters arrive as `{$oid}`
 * markers and are revived before this runs.
 */
